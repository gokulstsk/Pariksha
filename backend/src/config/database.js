...
const databaseUrl = rawDatabaseUrl ? rawDatabaseUrl.replace(/([?&])channel_binding=[^&]+(&|$)/, "$1").replace(/[?&]$/, "") : null;

const isPostgres = Boolean(isVercel || isQa || process.env.DB_DIALECT === "postgres" || (databaseUrl && (databaseUrl.startsWith("postgres://") || databaseUrl.startsWith("postgresql://"))));
